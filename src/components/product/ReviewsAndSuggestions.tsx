...
import { Star, ThumbsUp, Camera, X, Loader2, User, Heart, ChevronRight } from "lucide-react";
...
interface ReviewsAndSuggestionsProps {
  productId: string;
  category: string;
}
        ...
        {products.map((product) => (
          ...
          >
            <div className="relative aspect-square overflow-hidden">
              ...
              >
                <Heart className={cn("w-4 h-4", isFavorite(product.id) ? "fill-destructive text-destructive" : "text-muted-foreground")} />
              </button>
            </div>
            <div className="p-3">
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <span className="text-xs text-muted-foreground">4.5 (50+)</span>
              </div>
              <p className="font-bold mt-1">₹{product.base_price}</p>
            </div>
          </Link>
        ))}
      </div>
...
// Main Component
export const ReviewsAndSuggestions = ({ productId, category }: ReviewsAndSuggestionsProps) => {
  ...
  const loadReviews = async () => {
    setLoading(true);
    try {
      // Try to load reviews - productId might be text-based for customize pages
      const { data, error } = await supabase
        .from('reviews')
        .select('*')
      ...
      if (error) {
        console.log('Reviews query info:', error);
        setReviews([]);
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="space-y-12 py-8 border-t border-border mt-8">
      {/* Reviews Section */}
