...
export const YouTubeSection = () => {
  ...
  const [channelStats, setChannelStats] = useState({
    subscribers: "50K+", videos_count: "200+", total_views: "5M+",
    ...
    const fetchStats = async () => {
      ...
      if (data && data.length > 0) {
        ...
        setChannelStats(prev => ({
          subscribers: map.yt_subscribers || prev.subscribers,
          videos_count: map.yt_videos_count || prev.videos_count,
          total_views: map.yt_total_views || prev.total_views,
          channel_url: map.yt_channel_url || prev.channel_url,
        }));
      }
    };
    fetchVideos();
    ...
    <section className="py-16 bg-card">
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
          ...
          <Button size="lg" className="bg-red-600 hover:bg-red-700 text-white" onClick={() => window.open(channelStats.channel_url, "_blank")}>
        ...
        <div className="mt-12 bg-gradient-to-r from-red-600 to-red-500 rounded-2xl p-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div className="flex items-center gap-8">
              <div className="text-center"><p className="text-3xl font-bold text-white">{channelStats.subscribers}</p><p className="text-sm text-white/70">Subscribers</p></div>
              <div className="text-center"><p className="text-3xl font-bold text-white">{channelStats.videos_count}</p><p className="text-sm text-white/70">Videos</p></div>
              <div className="text-center"><p className="text-3xl font-bold text-white">{channelStats.total_views}</p><p className="text-sm text-white/70">Views</p></div>
            </div>
          </div>
        </div>
      </div>
