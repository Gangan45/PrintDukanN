...
import { Star, ShoppingCart, Pencil, Loader2 } from "lucide-react";
...
const NamePencilCategory = () => {
  ...
  // Use first product's data for pricing & images
  const mainProduct = products[0];
  const bulkPricing: { label: string; price: number }[] =
    (mainProduct?.sizes as any[])?.length > 0
      ? (mainProduct.sizes as any[]).map((s: any) => ({
          label: s.label || s.name || "",
          price: s.price,
        }))
      : fallbackBulkPricing;

  const heroImage = mainProduct?.images?.[0] || namePencil1;
  const basePrice = mainProduct?.base_price || bulkPricing[0]?.price || 299;
  const productName = mainProduct?.name || "Name Pencils";
  const productDescription =
    mainProduct?.description ||
    "Personalized name pencils that your kids will love! Perfect for schools, return gifts & corporate events.";

  // Use product images for features if available
  const featureImages = mainProduct?.images?.length >= 4
    ? mainProduct.images.slice(0, 4)
    : [namePencil1, namePencil2, namePencil3, namePencil4];

  const features = fallbackFeatures.map((f, i) => ({
    ...f,
    image: featureImages[i] || f.image,
  }));
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-background">
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div className="space-y-6">
                ...
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-foreground leading-tight">
                  {productName.split(" ").map((word, i) =>
                    i === 0 ? (
                      ...
                      <span key={i} className="text-amber-600 dark:text-amber-400">{word} </span>
                    )
                  )}
                </h1>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 text-sm font-semibold px-3 py-1 rounded-full">
                    {Math.round((1 - basePrice / Math.round(basePrice * 1.25)) * 100)}% OFF
                  </span>
                </div>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (mainProduct) navigate(`/name-pencil/${mainProduct.id}`);
              ...
              <div className="relative">
                <img src={heroImage} alt={productName} className="w-full rounded-2xl shadow-2xl" />
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {bulkPricing.map((tier, i) => (
                <button
                  ...
                  className={`relative rounded-xl p-4 text-center border-2 transition-all cursor-pointer hover:scale-105 hover:shadow-md ${
                    i === 0
                      ? "border-amber-500 bg-amber-50 dark:bg-amber-950/20"
                      : "border-border bg-card hover:border-amber-300"
                  }`}
                >
                  {i === bulkPricing.length - 1 && (
                  ...
                  <p className="text-xs text-muted-foreground">₹{(tier.price / 10).toFixed(1)}/pencil</p>
                </button>
              ))}
            </div>
          </div>
        ...
        {/* Products Grid - show all Name Pencil products from DB */}
        {products.length > 1 && (
          <section className="py-12 md:py-16 bg-background">
            <div className="container mx-auto px-4">
              <h2 className="text-3xl font-bold text-center text-foreground mb-8">Our Name Pencil Collection</h2>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {products.map((product) => (
                  <button
                    key={product.id}
                    onClick={() => navigate(`/name-pencil/${product.id}`)}
                    className="bg-card rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all border border-border text-left group"
                  >
                    <div className="aspect-square overflow-hidden">
                      <img
                        src={product.images?.[0] || namePencil1}
                        alt={product.name}
                    ...
                    <div className="p-3">
                      ...
                      <p className="text-amber-600 font-bold">₹{product.base_price}</p>
                    </div>
                  </button>
                ))}
              </div>
        ...
        <section className="py-12 md:py-16 bg-background">
          <div className="container mx-auto px-4 text-center">
            ...
            >
              ...
              Order Now - Starting ₹{bulkPricing[bulkPricing.length - 1]?.price || 79}/pack
            </Button>
          </div>
        </section>
      </main>
