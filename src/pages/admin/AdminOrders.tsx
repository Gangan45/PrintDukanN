  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const enrichedOrders = ordersData?.map(order => ({
        ...order,
        customer_email: order.user_id ? (profileMap.get(order.user_id)?.email || "N/A") : (order.guest_email || "Guest"),
        customer_name: order.user_id ? (profileMap.get(order.user_id)?.full_name || "Customer") : (order.guest_name || "Guest Customer"),
      })) || [];
