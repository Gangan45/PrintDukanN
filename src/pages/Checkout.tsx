...
const Checkout = () => {
  ...
  // Fetch dynamic COD charges from product if Name Pencils in cart
  useEffect(() => {
    const fetchCodCharges = async () => {
      const namePencilItem = checkoutItems.find(item => item.category === "Name Pencils" && item.product_id);
      if (namePencilItem && namePencilItem.product_id) {
        const { data } = await supabase
          .from("products")
          .select("frames")
          .eq("id", namePencilItem.product_id)
          .maybeSingle();
        if (data?.frames) {
          const frames = data.frames as any;
          if (typeof frames.cod_charges === "number") {
            setCodAdvanceAmount(frames.cod_charges);
            return;
          }
        }
      }
      setCodAdvanceAmount(DEFAULT_COD_ADVANCE);
    };
    ...
    try {
      if (paymentMethod === "razorpay") {
        ...
        if (paymentResult.success) {
          ...
          setOrderNumber(orderResult.newOrderNumber);
          setOrderComplete(true);
          toast({ title: "Success", description: `Advance payment of ₹${codAdvanceAmount} received! Order confirmed. Pay ₹${Math.max(0, finalTotal - codAdvanceAmount).toLocaleString()} on delivery.` });
        } else {
          if (paymentResult.error !== "Payment cancelled by user") {
                    ...
                    >
                      ...
                      <Label htmlFor="cod" className="flex items-center gap-3 cursor-pointer flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">Pay ₹{codAdvanceAmount} advance, rest on delivery</p>
                        </div>
                      </Label>
                    </div>
                  </RadioGroup>
                  ...
                  {paymentMethod === "cod" && (
                    <div className="p-4 bg-amber-50 dark:bg-amber-500/10 rounded-xl border border-amber-200 dark:border-amber-500/20">
                      ...
                      <p className="text-xs text-amber-700 dark:text-amber-400/80">
                        To confirm your COD order, please pay ₹{codAdvanceAmount} advance. The remaining ₹{Math.max(0, finalTotal - codAdvanceAmount).toLocaleString()} will be collected on delivery.
                      </p>
                    </div>
                  )}
                  ...
                  {paymentMethod === "cod" && (
                    <div className="mt-2 p-3 bg-amber-500/10 rounded-xl border border-amber-500/20">
                      <p className="text-xs text-amber-700 dark:text-amber-400 font-medium">
                        COD requires ₹{codAdvanceAmount} advance payment. Remaining ₹{Math.max(0, finalTotal - codAdvanceAmount).toLocaleString()} payable on delivery.
                      </p>
                    </div>
                  )}
                </div>
